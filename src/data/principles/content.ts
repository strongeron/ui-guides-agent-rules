...
export const contentPrinciples: Principle[] = [
  ...
  {
    id: 'content-animated-text-stays-readable',
    category: 'content',
    source: 'custom',
    title: 'Animated Text Stays Readable',
    description: 'Splitting text into per-letter or per-word spans for animation must not destroy its accessible name',
    sourceQuote: 'A word split into a span per letter is, to assistive tech, a list of single characters — not a word. Keep the accessible name on the wrapper and hide the shards.',
    additionalExplanation: 'Staggered letter/word reveals are built by wrapping each glyph in its own element. Visually it is still a word; to a screen reader it is ten unrelated characters read "A, n, n, o…", or dropped entirely — and the same fragmentation breaks find-in-page, translation, and text selection. The fix costs nothing: put the real text as an aria-label on the wrapper and mark every animated shard aria-hidden="true", so the DOM animates while the accessibility tree sees one intact word. Pair it with reduced-motion — under prefers-reduced-motion the reveal should drop to a plain fade or appear instantly. This is WCAG 1.3.1 (Info and Relationships) at the glyph level: the visual grouping into a word must survive into the semantics.',
    sourceLinks: [
      { text: 'WCAG 1.3.1 — Info and Relationships', url: 'https://www.w3.org/WAI/WCAG21/Understanding/info-and-relationships.html' },
      { text: 'MDN — aria-label', url: 'https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/Attributes/aria-label' },
    ],
    tags: ['typography', 'a11y', 'motion'],
    badExampleKey: 'content-animated-text-stays-readable-bad',
    goodExampleKey: 'content-animated-text-stays-readable-good',
  },
  {
    id: 'content-moving-text-can-be-paused',
    category: 'content',
    source: 'wcag',
    title: 'Moving Text Can Be Paused',
    description: 'Auto-scrolling tickers, marquees, and looping text need a pause control and must honor reduced-motion',
    sourceQuote: 'WCAG 2.2.2: For any moving, blinking, or scrolling information that starts automatically, lasts more than five seconds, and is presented in parallel with other content, there is a mechanism to pause, stop, or hide it.',
    additionalExplanation: 'A marquee or news ticker that scrolls on its own is a moving target the reader cannot catch — worse for anyone with a vestibular disorder, low vision, or a reading disability, for whom the movement is not decoration but an obstacle. WCAG 2.2.2 makes it concrete: anything that moves automatically for more than five seconds alongside other content must offer a pause, stop, or hide control. Two things satisfy it together: a visible Pause button (toggle animation-play-state, and expose aria-pressed), and a prefers-reduced-motion: reduce rule that stops the animation entirely for users who asked the OS to. Auto-motion is a convenience you offer, never one you impose.',
    sourceLinks: [
      { text: 'WCAG 2.2.2 — Pause, Stop, Hide', url: 'https://www.w3.org/WAI/WCAG21/Understanding/pause-stop-hide.html' },
      { text: 'MDN — prefers-reduced-motion', url: 'https://developer.mozilla.org/en-US/docs/Web/CSS/@media/prefers-reduced-motion' },
    ],
    tags: ['typography', 'a11y', 'motion'],
    badExampleKey: 'content-moving-text-can-be-paused-bad',
    goodExampleKey: 'content-moving-text-can-be-paused-good',
  },
];
