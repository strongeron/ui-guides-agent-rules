...
export const designPrinciples: Principle[] = [
  ...
  },
  {
    ...
    sourceLinks: [
      ...
      { text: 'emilkowalski/skills — apple-design', url: 'https://github.com/emilkowalski/skills/tree/main/skills/apple-design' },
    ],
  ...
  {
    id: 'design-flip-card-depth',
    category: 'design',
    source: 'web-platform',
    title: 'Real Depth Needs preserve-3d',
    description: 'A flip card or any true 3D transform needs transform-style: preserve-3d and backface-visibility on the faces',
    sourceQuote: 'transform-style: preserve-3d keeps a transformed element\'s children in the same 3D space; backface-visibility: hidden stops a face from showing through when it turns away. Without them, rotateY only mirror-flips a flat element.',
    additionalExplanation: 'The instinct for a flip card is to slap rotateY(180deg) on an element and call it 3D. It is not: with the default transform-style: flat, the element is a flat picture that turns edge-on and reappears mirrored — your text reads backwards and there is no back. Three properties make it real. perspective on the container gives the scene a vanishing point (see "Perspective on the Parent"). transform-style: preserve-3d on the rotating element keeps its two faces positioned in 3D rather than flattened onto one plane. backface-visibility: hidden on each face hides it while it points away from the viewer, so the front and a separately-authored back swap cleanly. Because the flip is user-initiated and brief a short transition is fine, but still gate longer or ambient 3D motion behind prefers-reduced-motion (see "Reduce Ambient 3D Motion").',
    sourceLinks: [
      { text: 'MDN — transform-style', url: 'https://developer.mozilla.org/en-US/docs/Web/CSS/transform-style' },
      { text: 'MDN — backface-visibility', url: 'https://developer.mozilla.org/en-US/docs/Web/CSS/backface-visibility' },
    ],
    tags: ['design', '3d', 'motion'],
    badExampleKey: 'design-flip-card-depth-bad',
    goodExampleKey: 'design-flip-card-depth-good',
  },
  {
    id: 'design-perspective-on-parent',
    category: 'design',
    source: 'web-platform',
    title: 'Perspective on the Parent',
    description: 'Set perspective once on the shared container, not baked into each child\'s transform',
    sourceQuote: 'A single perspective on the parent element gives all its 3D children one shared vanishing point. perspective() written into each child\'s transform gives every child its own — the scene stops being coherent.',
    additionalExplanation: 'There are two ways to add depth and they are not equivalent. The perspective property on a container establishes one viewpoint for everything inside it, so a row of rotated cards converges toward a single point and reads as one shelf in one space. Writing the perspective() function into each child\'s own transform instead re-centres the viewpoint on that child, so every element is seen head-on from its own middle and the group looks flat and unrelated no matter how far you rotate it. The rule of thumb: perspective (the property) belongs on the nearest common ancestor of the 3D elements that should share a scene; reach for the perspective() function only when you deliberately want one element to have an independent viewpoint. Tune the value to taste — smaller is a more dramatic, wide-angle depth.',
    sourceLinks: [
      { text: 'MDN — perspective', url: 'https://developer.mozilla.org/en-US/docs/Web/CSS/perspective' },
      { text: 'MDN — Using CSS transforms (3D)', url: 'https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_transforms/Using_CSS_transforms' },
    ],
    tags: ['design', '3d'],
    badExampleKey: 'design-perspective-on-parent-bad',
    goodExampleKey: 'design-perspective-on-parent-good',
  },
  {
    id: 'design-compound-over-mega-component',
    category: 'design',
    source: 'vercel-composition',
    title: 'Compound Parts Over a Mega-Component',
    description: 'Expose a component as composable parts (Select.Trigger / Select.Content) rather than one component with a dozen configuration props',
    sourceQuote: 'Prefer compound components over a single component with many props. When a component needs a header, a divider, or a custom row, that should be a child — not another prop and another branch inside the component.',
    additionalExplanation: 'A component that starts as `<Select options value onChange>` grows a new prop every time a caller needs something slightly different — a label, an icon, a sticky header, a custom empty state — until it is a maze of props and internal conditionals that no one can safely change. Compound components invert this: the parent owns state and context, and the caller composes the pieces (`<Select><SelectTrigger/><SelectContent><SelectItem/></SelectContent></Select>`). New requirements become new children, not new props, so the component stays closed for modification but open for extension. This is the pattern shadcn/ui and Radix are built on, and it is why they rarely need a breaking change to support a new layout. Reserve the flat single-prop API for genuinely atomic things (an icon, a badge) that have no inside.',
    sourceLinks: [
      { text: 'Vercel — composition-patterns skill', url: 'https://github.com/vercel-labs/agent-skills/tree/main/skills/composition-patterns' },
      { text: 'Radix — Composition', url: 'https://www.radix-ui.com/primitives/docs/guides/composition' },
    ],
    tags: ['design', 'interface'],
    badExampleKey: 'design-compound-over-mega-component-bad',
    goodExampleKey: 'design-compound-over-mega-component-good',
  },
  {
    id: 'design-explicit-variants-over-booleans',
    category: 'design',
    source: 'vercel-composition',
    title: 'Explicit Variants Over Boolean Props',
    description: 'Model mutually-exclusive styles as one enumerated variant prop, not a pile of isPrimary/isGhost/isLarge booleans',
    sourceQuote: 'Use an explicit variant prop instead of multiple boolean props. Booleans like isPrimary, isSecondary, isGhost allow illegal combinations and multiply with every new style; a single variant makes the valid set explicit.',
    additionalExplanation: 'Three boolean style props already encode eight states, most of them nonsense — `isPrimary` and `isGhost` both true has no meaning, yet the type permits it and some branch has to guess. An enumerated prop, `variant: "primary" | "secondary" | "ghost"`, makes exactly the valid states representable and the illegal ones unspellable, which is the whole point of a type. It also maps one-to-one onto class-variance-authority, the engine shadcn/ui uses to attach classes per variant, so the API and the styling share one source of truth. Keep booleans for genuinely independent, orthogonal toggles (`disabled`, `loading`, `fullWidth`) — the test is whether two of them being true at once is meaningful. If it is not, they are really one variant wearing several names.',
    sourceLinks: [
      { text: 'Vercel — composition-patterns skill', url: 'https://github.com/vercel-labs/agent-skills/tree/main/skills/composition-patterns' },
      { text: 'class-variance-authority', url: 'https://cva.style/docs' },
    ],
    tags: ['design', 'interface'],
    badExampleKey: 'design-explicit-variants-over-booleans-bad',
    goodExampleKey: 'design-explicit-variants-over-booleans-good',
  },
  {
    id: 'design-render-as-child',
    category: 'design',
    source: 'vercel-composition',
    title: 'Let a Primitive Render as Its Child',
    description: 'Give styled primitives an asChild / Slot escape hatch so a Button can BE a link instead of wrapping one',
    sourceQuote: 'Support rendering as the child element (asChild / Slot) so a styled primitive can adopt the semantics of whatever it wraps — a Button that is actually an anchor — instead of forcing a fixed tag or nesting interactive elements.',
    additionalExplanation: 'A `<Button>` that only ever renders a `<button>` forces a bad choice the moment you need it to navigate: either fake a link with an onClick (losing href, middle-click, open-in-new-tab, and the right role) or wrap it in an `<a>` and nest two interactive elements, which is invalid and confuses assistive tech. The `asChild` pattern — Radix\'s Slot — resolves it by merging the primitive\'s classes, refs, and event handlers onto the single child you pass, so `<Button asChild><a href>` renders one real anchor that looks and behaves like the button. It keeps styling and semantics in separate hands: the primitive owns the look, the child owns what the element actually IS. This is how shadcn/ui buttons become links, menu items become `<Link>`s, and triggers wrap arbitrary controls without a prop explosion.',
    sourceLinks: [
      { text: 'Vercel — composition-patterns skill', url: 'https://github.com/vercel-labs/agent-skills/tree/main/skills/composition-patterns' },
      { text: 'Radix — asChild / Slot', url: 'https://www.radix-ui.com/primitives/docs/guides/composition' },
    ],
    tags: ['design', 'interface', 'a11y'],
    badExampleKey: 'design-render-as-child-bad',
    goodExampleKey: 'design-render-as-child-good',
  },
];
