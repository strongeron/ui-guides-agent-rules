...
export const animationsPrinciples: Principle[] = [
  ...
  },
  {
    ...
    sourceQuote: 'MUST animate only compositor props (`transform`, `opacity`)',
    additionalExplanation: 'transform and opacity are the only two properties the browser can change without redoing style, layout or paint — the compositor already holds the layer and just re-places or re-blends it, on its own thread. That is why they keep moving at 60fps even while the main thread is blocked, and why everything else stutters the moment it is not. ibelick\'s own fixing-motion-performance skill states the same rule with its exceptions attached: "paint or layout animation is acceptable only on small, isolated surfaces" and "one-shot effects are acceptable more often than continuous motion" — so read this MUST as the default, and the size/duration carve-outs (animations-ibelick-minimize-paint, performance-ibelick-no-blur-animation) as the priced exits from it.',
    sourceLinks: [
      { text: 'ibelick — baseline-ui SKILL.md', url: 'https://github.com/ibelick/ui-skills/blob/main/skills/baseline-ui/SKILL.md' },
  ...
  },
  {
    ...
    sourceQuote: 'SHOULD respect `prefers-reduced-motion`',
    additionalExplanation: 'Some users experience motion sickness or vestibular symptoms from movement they did not initiate. The media query surfaces an OS-level preference they have already expressed, so honouring it costs one @media block. "Reduced" is not "none": keep opacity crossfades and instant state changes so the interface still communicates what happened — see animations-lottie-never-opacity-only for the corollary, that a state change reinforced by motion must still be legible once the motion is dropped.',
    sourceLinks: [
      { text: 'ibelick — baseline-ui SKILL.md', url: 'https://github.com/ibelick/ui-skills/blob/main/skills/baseline-ui/SKILL.md' },
